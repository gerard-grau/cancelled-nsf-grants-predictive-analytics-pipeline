...
En aquesta secció documentem el procés d'entrenament dels models de machine learning per predir si una subvenció formarà part de la llista de Cruz (variable objectiu \texttt{in\_cruz\_list}).

\subsection{Enfocament del problema}

El problema plantejat és una tasca de \textbf{classificació binària}: predir si una subvenció de la NSF serà inclosa a la llista de Cruz (\texttt{in\_cruz\_list = True}) o no (\texttt{in\_cruz\_list = False}).

\textbf{Desequilibri de classes:} El dataset presenta un fort desequilibri, amb significativament més subvencions que no formen part de la llista de Cruz. Aquest desequilibri pot afectar el rendiment dels models, ja que tendeixen a predir la classe majoritària per maximitzar l'accuracy.

\textbf{Estratègia adoptada:} Per abordar aquest desequilibri, hem aplicat \textbf{pesos de classe} (\textit{class weights}) als models, de manera que les mostres de la classe minoritària tinguin més importància durant l'entrenament. A més, utilitzem mètriques com F1-score i AUC-ROC que són més robustes davant datasets desequilibrats que la simple accuracy.
...
\begin{itemize}
    \item \textbf{Features numèriques:} 
        \begin{itemize}
            ...
            \item \texttt{log\_estimated\_total}: transformació logarítmica de l'import
            \item \texttt{has\_flagged\_words}: nombre de paraules clau sensibles al títol/abstract
            \item \texttt{flagged\_word\_count}: comptatge de paraules clau
            \item \texttt{flagged\_word\_density}: densitat de paraules clau al text
        \end{itemize}
    ...
    \item \textbf{Features booleanes:}
        \begin{itemize}
            \item \texttt{is\_terminated}: si la subvenció ha estat cancel·lada
        \end{itemize}
\end{itemize}
...
Genera prediccions aleatòries seguint la distribució observada al dataset d'entrenament. Punt de referència mínim per validar que els models aprenen dels datos.

\textbf{2. Majority Class Baseline:}
Prediu sempre la classe majoritària. Estableix l'accuracy mínim esperable ignorant completament les features del dataset.

\textbf{3. Stratified Baseline:}
Combina estratègia aleatòria mantenint la distribució de classes. Validació addicional del preprocessament de dades.
...
\textbf{Observacions:}
\begin{itemize}
    \item Els models de ML superen clarament els baselines, validant l'eficàcia de les features extretes durant el preprocessament.
    \item Gradient Boosted Trees obté el millor F1-score (0.1063), indicant un millor equilibri entre precision i recall per a la classe minoritària.
    \item Els AUC-ROC superiors a 0.70 demostren bona capacitat de discriminació entre classes.
    \item Els resultats són consistents entre validació i test, confirmant bona generalització.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{../scripts/images/roc_curves.png}
...
\textbf{Criteri de selecció:} Hem utilitzat el \textbf{F1-score} com a mètrica principal per seleccionar el millor model, ja que equilibra precision i recall, cosa especialment important en datasets desequilibrats com el nostre (només l'1.4\% de les subvencions formen part de la llista de Cruz).

\textbf{Model seleccionat:} Gradient Boosted Trees amb un F1-score de 0.1063 al conjunt de test.

\textbf{Justificació:}
\begin{itemize}
    \item Millor F1-score entre tots els models, indicant millor detecció de la classe minoritària
    \item AUC-ROC de 0.7159, demostrant bona capacitat de discriminació
    \item Millora significativa respecte als baselines (F1 de 0.1063 vs 0.0488 del millor baseline)
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{../scripts/images/confusion_matrices.png}
