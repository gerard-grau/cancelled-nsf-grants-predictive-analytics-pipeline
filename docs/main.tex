...
El sistema utilitza configuracions explícites per dataset (\texttt{FormatterConfig}) que defineixen mapejaments de columnes, formats de dates i validacions específiques per a cada font de dades. El flux de processament aplica operacions de renomenat de columnes (per exemple, conversió de camelCase a snake\_case), parsejat de dates amb formats específics, conversió de tipus numèrics i booleeans, i transformacions personalitzades quan és necessari (com la neteja de valors categòrics nuls o el filtratge de paraules invàlides). Un cop processades, les dades es carreguen a MongoDB amb índexs optimitzats sobre els camps clau (\texttt{award\_id}, \texttt{award\_number}, etc.) per facilitar l'accés en fases posteriors.
