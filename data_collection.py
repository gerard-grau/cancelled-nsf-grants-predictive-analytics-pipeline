...
def read_terminated(file_path, spark: SparkSession | None = None):
    ...
    if spark is not None:
        ...
        parquet_path = file_path.with_suffix("")  # mateix nom, sense .csv
        (
            df.write
            .mode("overwrite")
...
def download_legislators(file_path, spark: SparkSession | None = None):
    file_path = Path(file_path)
    file_path.parent.mkdir(parents=True, exist_ok=True)
...
def download_cruz_list(file_path, spark: SparkSession | None = None):
    file_path = Path(file_path)
    file_path.parent.mkdir(parents=True, exist_ok=True)
